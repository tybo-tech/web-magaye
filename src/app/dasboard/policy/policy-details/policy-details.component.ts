...
import { MEMBERTYPES, POLICY, COMPANYID, PREMIUM, BENEFIT, PREMIUM_BENEFIT } from 'src/app/shared/constants';
    ...
    this.itemService.getItems(COMPANYID, BENEFIT, false).subscribe(data => {
      ...
      if (data && data.length) {
        ...
        this.item.Children.forEach(itemOption => {
          const benefit = data.find(x => x.ItemId === itemOption.RelatedId);
          if (benefit)
